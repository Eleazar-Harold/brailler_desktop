from tkinter import END, Button, Frame, Label, Menu, Text, Tk

from src import BrailleToText, TextToBraille
...
class TextBraillePage(Frame):
    def __init__(self, parent, controller=None):
        ...
        self.button3 = Button(
            ...
            command=lambda: controller.show_frame(BrailleTextPage),
        )
        self.button3.pack(side="left", padx=15, pady=15)

    def click_ok(self):
        self.converted_input.insert(
            END, TextToBraille.t2b_translate(str(self.user_input.get(1.0, END)))
        )
...
class BrailleTextPage(Frame):
    def __init__(self, parent, controller):
        ...
        self.button3 = Button(
            ...
            command=lambda: controller.show_frame(TextBraillePage),
        )
        self.button3.pack(side="left", padx=15, pady=15)

    def click_ok(self):
        self.converted_input.insert(
            END, BrailleToText.translate(str(self.user_input.get(1.0, END)))
        )
        # self.converted_input.config(state=DISABLED)
